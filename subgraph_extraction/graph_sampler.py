...
def sample_links(links, sample_ratio):
    """
    Randomly sample a subset of links.
    """
    links_list = list(links)  # Convert to list if it's not already
    sample_size = int(len(links_list) * sample_ratio)
    sampled_links = random.sample(links_list, sample_size)
...
#     num_pruned_nodes = []
#
#     subgraph_count = 0  # Counter for subgraph extraction
#     target_nodes_subgraphs_count = 0  # Counter for subgraphs containing more than two target nodes
#
...
#         nonlocal subgraph_count, target_nodes_subgraphs_count  # Use the counters from the outer function scope
#
...
#
#                 subgraph_count += 1  # Increment subgraph count after processing each link
#
#                 # Check if the subgraph contains more than two target node
#                 if len(set(datum['nodes']) - {datum['nodes'][0], datum['nodes'][1]}) > 0:
#                     target_nodes_subgraphs_count += 1
#
#
#
#     for split_name, split in graphs.items():
...
#
#     print("number of subgraphs:", subgraph_count)  # Return the total number of extracted subgraphs
#     print("number of subgraphs containing more than two target nodes:", target_nodes_subgraphs_count)

def links2subgraphs(A, graphs, params, max_label_value=None, sample_ratio=1):
    '''
    extract enclosing subgraphs, write map mode + named dbs
    '''
    max_n_label = {'value': np.array([0, 0])}
    ...
    for split_name, split in graphs.items():
        print(f"Extracting enclosing subgraphs for positive links in {split_name} set")
        sampled_pos_links = sample_links(split['pos'], sample_ratio)
        labels = np.ones(len(sampled_pos_links))
        db_name_pos = split_name + '_pos'
        split_env = env.open_db(db_name_pos.encode())
        extraction_helper(A, sampled_pos_links, labels, split_env)

        print(f"Extracting enclosing subgraphs for negative links in {split_name} set")
        sampled_neg_links = sample_links(split['neg'], sample_ratio)
        labels = np.zeros(len(sampled_neg_links))
        db_name_neg = split_name + '_neg'
        split_env = env.open_db(db_name_neg.encode())
        extraction_helper(A, sampled_neg_links, labels, split_env)
